...
fun makeCaption(gpxAnalyzeResult: GPXAnalyzeResult?, defaultTripName: String): String = buildString {
    gpxAnalyzeResult?.let { result ->
        ...
        result.duration?.let { append("<b>⏰ Duration:</b> <i>${DurationFormatUtils.formatDurationHMS(it.toMillis())}</i>\n") }
        ...
        result.ascent.totalDuration?.let {
            append(
                "<b>↗️ Total (duration):</b> <i>${
        ...
        result.ascent.maxDuration?.let {
            append(
                "<b>↗️ Longest section (duration):</b> <i>${
                    DurationFormatUtils.formatDurationHMS(
                        it.toMillis()
                    )
                }</i>\n"
        ...
        result.descent.totalDuration?.let {
            append(
                "<b>↘️ Total (duration):</b> <i>${
                    DurationFormatUtils.formatDurationHMS(
                        it.toMillis()
        ...
        result.descent.maxDuration?.let {
            append(
                "<b>↘️ Longest section (duration):</b> <i>${
                    DurationFormatUtils.formatDurationHMS(
                        it.toMillis()
